...
lista_files <- function(wind, mov){
  general_path = 'C:/Users/Javier/Documents/MEGA/Universitattt/Master/Thesis/CDS_data/Sliced_ep_draw'
  directory = paste('Slice',as.character(wind),as.character(mov), sep = '_')
  path = paste(general_path, directory, sep = '/')
...
compute_prob <- function(node_names, inst_def, parameters){
...
table_prob_v2 <- function(prob){
  ...
  for (r in row.names(prob)){
    ...
    for (c in colnames(prob)){
      cat(paste(' & ', as.numeric(format(prob[r,c], digits = 6))))
    }
    cat('\\\\\n\\hline\n')
  }
}
...
mont.carl.BN <- function(Data, score, bootstrap, algthm, param, iter_mont){
  counterparties = c('AKT','ALROSA','BKECON','BOM','CITMOS','GAZPRU',
                     'GAZPRU.Gneft','LUKOIL','MBT','MDMOJC','ROSNEF','RSBZAO',
                     'RUSAGB','RUSRAI','SBERBANK','VIP','VTB','EVRGSA')
  ...
  adjmat = as.data.frame(matrix(0, 19, 19), row.names = append(counterparties,'RUSSIA'))
  ...
  return(list(prob_bayes, prob_mle, adjmat))
}
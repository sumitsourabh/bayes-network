...
def calculate_sd_epsilon(local_index, entity_ts, var_param):
    ...
    local_index: List of time series of the entity, index of local minima and local maxima
    ...
    for index in local_index:
        if index < var_param:
            epsilon_list.append(np.std(entity_ts[0:var_param], ddof=1))
        else:
            epsilon_list.append(np.std(entity_ts[index - var_param + 1: index + 1], ddof=1))
    return epsilon_list
...
def compute_epsilon_drawup(entity_ts, epsilon_choice, choose_drawup):
    """
    ...
    choose_drawup: string parameter corresponding to min or max epsilon drawup
    ...
    if (len(local_min_index) == 0) & (len(local_max_index) == 0):
        return []
    else:
        ...
        if local_min_index[0] < local_max_index[0]:
            return calibrate_epsilon_drawup_ts(entity_ts, epsilon_up_list, epsilon_down_list,
                                               local_min_index, local_max_index, 0, choose_drawup)
        else:
            return calibrate_epsilon_drawup_ts(entity_ts, epsilon_up_list, epsilon_down_list,
                                               local_min_index, local_max_index, 1, choose_drawup)


def calibrate_epsilon_drawup_ts(entity_ts, epsilon_up_list, epsilon_down_list, local_min_index,
                                local_max_index, diff, choose_drawup):
    """
    ...
    diff: difference parameter for ordering local minima and maxima
    choose_drawup: epsilon choice parameter for standard deviation or percentage
    ...
    i = 0
    j = i + diff
    while i < len(local_min_index) - (2 - diff):
        epsilon_min_candidate = i
        if (entity_ts[local_max_index[j]] - entity_ts[local_min_index[j + (1 - diff)]]) < \
                epsilon_down_list[j]:
            j += 1
        else:
            if entity_ts[local_max_index[j]] - entity_ts[local_min_index
              [epsilon_min_candidate]] > epsilon_up_list[epsilon_min_candidate]:
                if choose_drawup == 'max':
                    epsilon_drawup_list.append(local_max_index[j])
                elif choose_drawup == 'min':
                    epsilon_drawup_list.append(local_min_index[epsilon_min_candidate])
                    j += 1
            else:
                j += 1
        i = j
...
def epsilon_drawup_network(entities_np, time_lag, market_ts, epsilon_choice,
    ...
    for i in range(np.size(entities_list)):
        epsilon_drawup_list = compute_epsilon_drawup(entities_np[i, 1:], epsilon_choice, choose_drawup)
...
def filter_market_effect(epsilon_drawup_matrix, market_ts, epsilon_choice, choose_drawup, time_lag):
    """
    ...
    choose_drawup: array, parameter for min or max draw-up
    ...
    market_epsilon_drawup_list = compute_epsilon_drawup(market_ts, epsilon_choice, choose_drawup)
    if len(market_epsilon_drawup_list) == 0:
        ...
        for i in range(len(epsilon_drawup_matrix)):
            ...
            entity_ts_corrected = diff(entity_ts, market_drawup_all)
...
def create_weight_matrix(epsilon_drawup_matrix, adj_matrix, time_lag, len_entity_ts):
    ...
    for i in range(rows):
        for j in range(rows):
            if adj_matrix[i, j] - (len(epsilon_drawup_matrix[i][:]) * len(time_lag) / len_entity_ts) > 0:
...
def compute_countryrank(weight_matrix, entities_list, stressed_list, initial_stress, draw_network=False):
    """
    ...
    while len(visited_list) < len(entities_list):
        ...
        reachable_list = diff(reachable_list, visited_list)
    ...
    if draw_network:
        ut.draw_countryrank_network(entities_list, country_rank, visited_list, edge_list, edge_weight)
